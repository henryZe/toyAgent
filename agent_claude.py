...
def grep(pattern, path="."):
    try:
        result = subprocess.run(f"grep -r '{pattern}' {path}",
...
def run_agent_step(messages, tools, max_iterations=500):
    global current_plan, plan_mode
            ...
            if "_argument_error" in function_args:
                ...
                if current_plan:
                    ...
                    for i, step in enumerate(current_plan, 1):
                        ...
                        messages.append({"role": "user", "content": f"[Step {i}/{len(current_plan)}] {step}\n"
                                                                    f"Previous step result: {results[-1] if results else 'None'}"})
                ...
                function_response = f"Error: Can't support MCP tools '{function_name}'"
